...
export const projects = sqliteTable("projects", {
	...
	isGithubDisconnected: integer("is_github_disconnected").default(0).notNull(),
...
export const tasks = sqliteTable("tasks", {
	...
	dependencyTaskIdsJson: text("dependency_task_ids_json").default("[]").notNull(),
...
(table) => [
	uniqueIndex("github_repositories_installation_repo").on(table.installationId, table.githubRepoId),
]);

export const githubUserAuthorizations = sqliteTable("github_user_authorizations", {
	id: text().primaryKey().notNull(),
	userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" } ),
...
export const githubSyncedItems = sqliteTable("github_synced_items", {
	id: text().primaryKey().notNull(),
	workspaceId: text("workspace_id").notNull().references(() => workspaces.id, { onDelete: "cascade" } ),
	projectId: text("project_id").notNull().references(() => projects.id, { onDelete: "cascade" } ),
...
(table) => [
	uniqueIndex("github_synced_items_repo_node").on(table.githubRepoId, table.githubNodeId),
]);
...
export const onboardingSessions = sqliteTable("onboarding_sessions", {
	...
	status: text().default("INTERVIEWING").notNull(),
	docUrl: text("doc_url"),
	createdAt: integer("created_at").notNull(),
});
